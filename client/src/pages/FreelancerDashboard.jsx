...
const FreelancerDashboard = () => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      // If no token, redirect or show error (optional: handleLogout())
      if (!token) {
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-6 rounded-xl shadow-sm">
            <p className="text-gray-500">Available Projects</p>
            <h3 className="text-3xl font-bold text-blue-600 mt-2">
              {projects.length}
            </h3>
          </div>
          <div className="bg-white p-6 rounded-xl shadow-sm">
            <p className="text-gray-500">My Active Projects</p>
            <h3 className="text-3xl font-bold text-orange-500 mt-2">0</h3>
          </div>
          <div className="bg-white p-6 rounded-xl shadow-sm">
        ...
        {!loading && projects.length === 0 && !error && (
          <div className="bg-white p-10 rounded-xl text-center shadow-sm">
            ...
            <p className="text-gray-500 mt-2">There are currently no projects available.</p>
          </div>
        )}
            ...
            {projects.map((project) => (
              ...
              >
                <div className="flex justify-between items-start gap-3">
                  ...
                  <span className="text-sm px-3 py-1 rounded-full bg-green-100 text-green-700 whitespace-nowrap">
                    Open
                  </span>
                </div>
